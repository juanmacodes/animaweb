...
class RsTypewriterSliderFront extends RevSliderFunctions {
	...
	public function add_addon_files($html, $export){
		...
		$_jsPathMin = file_exists(RS_TYPEWRITER_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
		if(!$export->usepcl){
			$export->zip->addFile(RS_TYPEWRITER_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
		}else{
			$export->pclzip->add(RS_TYPEWRITER_PLUGIN_PATH.'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_TYPEWRITER_PLUGIN_PATH.'sr6/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
		}
	...
	private function isEnabled($slider){
		...
		return (!$this->isFalse($enabled)) ? true : false;
	}
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['typewriter'] = '<script'. $addition .' src="'. $this->pluginUrl . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
