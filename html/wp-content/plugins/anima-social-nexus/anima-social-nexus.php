...
function anima_nexus_enqueue_scripts()
{
	...
	// Esto es CRÃTICO para que AJAX funcione.
	wp_localize_script(
		'anima-nexus-script',
		'anima_nexus_vars',
		array(
...
// Registramos el shortcode. ESTA LÃNEA ES LA CLAVE.
...
function anima_nexus_post_comment()
{
	...
	if ($comment_id) {
		...
		if ($post && $post->post_author != $user->ID) {
			anima_send_onesignal_notification($post->post_author, "Nuevo comentario de " . $user->display_name, get_permalink($post_id));
		}
		wp_send_json_success();
	} else {
		wp_send_json_error();
...
/**
 * HELPER: Enviar NotificaciÃ³n OneSignal
 */
function anima_send_onesignal_notification($user_id, $message, $url)
{
	$os_settings = get_option('OneSignalWP_settings');
	$app_id = isset($os_settings['app_id']) ? $os_settings['app_id'] : '';
	$api_key = isset($os_settings['app_rest_api_key']) ? $os_settings['app_rest_api_key'] : '';
...
/**
 * NOTIFICACIÃ“N: Nuevo Curso Publicado
 */
add_action('transition_post_status', 'anima_nexus_course_notification', 10, 3);
function anima_nexus_course_notification($new_status, $old_status, $post) {
    if ( $post->post_type !== 'sfwd-courses' ) return;
    if ( $new_status === 'publish' && $old_status !== 'publish' ) {
        // Enviar a TODOS (Segmento 'Subscribed Users')
        $message = "ðŸŽ“ NUEVO CURSO: " . $post->post_title . " ya estÃ¡ disponible en la Academia.";
