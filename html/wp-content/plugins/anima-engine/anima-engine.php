...
final class Anima_Engine_Core {
	...
	private function load_includes() {
		...
		if ( file_exists( $cpt_file_inc ) ) require_once( $cpt_file_inc );

		$course_fields = ANIMA_ENGINE_PATH . 'inc/admin-course-fields.php';
		if ( file_exists( $course_fields ) ) require_once( $course_fields );
		
        $product_fields = ANIMA_ENGINE_PATH . 'inc/admin-product-fields.php';
        if ( file_exists( $product_fields ) ) require_once( $product_fields );
	}
...
if ( ! function_exists( 'anima_award_xp_on_purchase' ) ) {
    function anima_award_xp_on_purchase( $order_id ) {
        ...
        if ( get_post_meta( $order_id, '_anima_xp_awarded', true ) === 'yes' ) return; 
...
if ( ! function_exists( 'anima_assign_credits_on_purchase' ) ) {
    function anima_assign_credits_on_purchase( $order_id ) {
        if ( ! $order_id || ! class_exists( 'WooCommerce' ) ) return; 

        $credit_packages_config = anima_get_credit_packages();
        $order = wc_get_order( $order_id );
        $user_id = $order->get_user_id();

        if ( get_post_meta( $order_id, '_anima_credits_awarded', true ) === 'yes' ) return; 
...
if ( ! function_exists( 'anima_admin_credits_field' ) ) {
    function anima_admin_credits_field( $user ) {
        ...
        <table class="form-table">
            <tr><th>Saldo Actual:</th><td><strong style="color: #00F0FF;"><?php echo number_format($credits); ?> ⚡</strong></td></tr>
            <tr><th>Modificar Saldo (+/-)</th><td><input type="number" name="anima_credit_change" placeholder="Ej: 100 o -50"></td></tr>
        </table>
        <?php
    }
}
...
if ( ! function_exists( 'anima_save_admin_credits_field' ) ) {
    function anima_save_admin_credits_field( $user_id ) {
        if ( ! current_user_can( 'edit_users' ) || empty( $_POST['anima_credit_change'] ) ) return;
...
if ( ! function_exists( 'anima_spend_credits' ) ) {
    function anima_spend_credits( $user_id, $cost ) {
        if ( ! $user_id || $cost <= 0 ) return false;
        $current = (int) get_user_meta( $user_id, 'anima_credits', true );
...
if ( ! function_exists( 'anima_handle_save_bio' ) ) {
    function anima_handle_save_bio() {
        ...
        if ( ! is_user_logged_in() ) { ob_end_clean(); wp_send_json_error( 'No autorizado' ); }
...
if ( ! function_exists( 'anima_handle_chat_request' ) ) {
    function anima_handle_chat_request() {
        ...
        if ( is_wp_error( $response ) ) wp_send_json_error( ['reply' => 'Error conexión.'] );
...
if ( ! function_exists( 'anima_render_academy_orb' ) ) {
    function anima_render_academy_orb() {
        ...
        if ( ! is_singular( 'curso' ) && ! is_post_type_archive( 'curso' ) ) return;
        ?>
        <div id="anima-orb-wrapper">
            <div id="anima-chat-window" class="anima-chat-window">
                <div class="chat-header">
                    <div class="chat-status"><span class="status-dot"></span><span class="status-text">A.N.I.M.A. ONLINE</span></div>
                    <button id="close-chat-btn" class="close-btn">×</button>
                </div>
                <div id="chat-messages" class="chat-messages">
                    <div class="message bot"><div class="msg-content">Sistemas listos. ¿En qué puedo ayudarte, Agente?</div></div>
                </div>
            ...
            <div id="anima-orb-trigger" class="anima-orb"><div class="orb-core"></div><div class="orb-ring"></div></div>
        </div>
        <style>
            #anima-orb-wrapper { position: fixed; bottom: 30px; right: 30px; z-index: 9999; font-family: sans-serif; }
            .anima-orb { width: 60px; height: 60px; position: relative; cursor: pointer; }
            .orb-core { width: 100%; height: 100%; background: #00F0FF; border-radius: 50%; box-shadow: 0 0 20px #00F0FF; }
            .orb-ring { position: absolute; top: -5px; left: -5px; right: -5px; bottom: -5px; border: 2px solid #BC13FE; border-radius: 50%; animation: spin 2s linear infinite; }
            @keyframes spin { to { transform: rotate(360deg); } }
            .anima-chat-window { position: absolute; bottom: 80px; right: 0; width: 300px; height: 400px; background: #111; border: 1px solid #333; display: none; flex-direction: column; }
            .anima-chat-window.active { display: flex; }
            .chat-header { padding: 10px; background: #222; display: flex; justify-content: space-between; color: #fff; }
            .chat-messages { flex: 1; padding: 10px; overflow-y: auto; color: #ddd; }
            .chat-input-area { padding: 10px; display: flex; }
            #chat-input { flex: 1; background: #000; border: 1px solid #444; color: #fff; padding: 5px; }
            .message { margin-bottom: 10px; padding: 8px; background: #222; border-radius: 4px; }
    ...
    function anima_render_my_connections_shortcode() {
        ...
        <div class="anima-connections-grid-dashboard">
            <?php foreach ( $connections as $conn ) : 
                ...
                if ( ! $friend_data ) continue; // Si el usuario fue borrado, saltar
            ...
            ?>
                <div class="anima-friend-card">
                    ...
                    <div class="friend-actions">
                        <a href="<?php echo esc_url( get_author_posts_url( $friend_id ) ); ?>" class="anima-icon-btn" title="Ver Perfil">
        ...
        <style>
            ...
            .friend-avatar { position: relative; width: 40px; height: 40px; flex-shrink: 0; }
            .friend-avatar img { width: 100%; height: 100%; border-radius: 50%; object-fit: cover; border: 1px solid #555; }
            .online-indicator { position: absolute; bottom: 0; right: 0; width: 8px; height: 8px; background: #00FF94; border-radius: 50%; border: 1px solid #000; }
            
            .friend-info { flex-grow: 1; overflow: hidden; }
            .friend-info h4 { margin: 0; font-size: 0.95rem; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
            .friend-role { font-size: 0.75rem; color: #888; display: block; }
            
            .friend-actions { display: flex; gap: 5px; }
            .anima-icon-btn { color: #aaa; text-decoration: none; transition: color 0.2s; }
            .anima-icon-btn:hover { color: #00F0FF; }
        </style>
        <?php
        return ob_get_clean();
    }
    ...
    function anima_shortcode_my_connections() {
        ...
        <div class="anima-connections-grid">
            ...
            <div class="connection-card">
                ...
                <div class="friend-actions">
                    <a href="<?php echo esc_url( home_url('/mi-cuenta/?view=inbox&compose=' . $friend_id) ); ?>" class="action-icon" title="Enviar Mensaje">
...
if ( ! function_exists( 'anima_handle_nexus_like' ) ) {
    function anima_handle_nexus_like() {
        ...
        if ( ! is_array( $users_who_liked ) ) $users_who_liked = [];
...
if ( ! function_exists( 'anima_handle_post_comment' ) ) {
    function anima_handle_post_comment() {
        if ( ! is_user_logged_in() ) wp_send_json_error( 'Login requerido' );
        
        $post_id = (int) $_POST['post_id'];
        $content = sanitize_textarea_field( $_POST['content'] );
        $user = wp_get_current_user();

        if ( empty($content) ) wp_send_json_error( 'Mensaje vacío' );
...
if ( ! function_exists( 'anima_award_badge' ) ) {
    function anima_award_badge( $user_id, $badge_id ) {
        ...
        if ( in_array( $badge_id, $badges ) ) return false;
...
if ( ! function_exists( 'anima_update_mission_progress' ) ) {
    function anima_update_mission_progress( $user_id, $action_type, $amount = 1 ) {
        ...
        $match = false;
        if ( $mission['id'] === 'bio_gen_3' && $action_type === 'generate_bio' ) $match = true;
        if ( $mission['id'] === 'nexus_post_2' && $action_type === 'post_nexus' ) $match = true;

        if ( ! $match ) return;

        $progress = get_user_meta( $user_id, 'anima_weekly_progress_' . $mission['id'], true );
        if ( ! is_array( $progress ) ) $progress = ['count' => 0, 'completed' => false];
...
add_action( 'wp_ajax_anima_generate_avatar_img', function() {
    if(is_user_logged_in()) anima_award_badge( get_current_user_id(), 'visualizer' );
}, 9 );
...
if ( ! function_exists( 'anima_secure_video_stream' ) ) {
    function anima_secure_video_stream() {
        ...
        if ( isset( $_GET['anima_video'] ) && $_GET['anima_video'] == 'true' ) {
            ...
            if ( ! is_user_logged_in() ) wp_die( 'Acceso Denegado: Identifícate.', 403 );

            $course_id = isset( $_GET['course_id'] ) ? (int) $_GET['course_id'] : 0;
            $file_name = isset( $_GET['file'] ) ? sanitize_text_field( $_GET['file'] ) : '';
            $user_id   = get_current_user_id();

            if ( ! $course_id || ! $file_name ) wp_die( 'Datos corruptos.', 400 );
            ...
            if ( current_user_can('manage_options') ) $has_access = true;

            if ( ! $has_access ) wp_die( 'Acceso Denegado: No posees los credenciales para este archivo.', 403 );
            ...
            if ( ! file_exists( $file_path ) ) wp_die( 'Archivo no encontrado en el servidor (Error 404).', 404 );
