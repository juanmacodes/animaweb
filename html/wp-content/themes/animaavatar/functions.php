...
// =====================================================================
// 2. CARGAR SCRIPTS Y ESTILOS (VERSIÓN COMPLETA CON ASSETS)
// =====================================================================

function animaavatar_scripts() {
	// Definir ruta base y versión para caché
	$css_path = get_template_directory_uri() . '/assets/css/';
	$version  = '1.0.5'; 

	// --- A) FUENTES Y ESTILOS GLOBALES ---
	...
	// 5. Comunidad / Nexus CSS
	if ( is_page( array( 'comunidad', 'nexus', 'feed', 'tablon' ) ) || is_page_template( 'page-tablon.php' ) || is_page_template( 'page-template-nexus-feed.php' ) || is_post_type_archive( 'nexus_post' ) ) {
		wp_enqueue_style( 'animaavatar-community', $css_path . 'community.css', array( 'animaavatar-utilities' ), $version );
	}

	// 6. Cursos CSS
	if ( is_page( array( 'cursos', 'curso' ) ) || is_singular( array( 'curso', 'leccion', 'sfwd-courses', 'sfwd-lessons' ) ) || is_post_type_archive( 'curso' ) || is_post_type_archive( 'sfwd-courses' ) ) {
		wp_enqueue_style( 'animaavatar-course', $css_path . 'course.css', array( 'animaavatar-utilities' ), $version );
	}
	...
	if ( file_exists( get_template_directory() . '/js/navigation.js' ) ) {
		wp_enqueue_script( 'animaavatar-navigation', get_template_directory_uri() . '/js/navigation.js', array( 'jquery' ), $version, true );
	}
...
// =====================================================================
// 3. REGISTRAR ÁREAS DE WIDGETS (SIDEBARS)
// =====================================================================
	...
	function anima_nexus_post_type_callback( $post ) {
		...
		<p>
			...
			<select name="anima_nexus_message_type" id="anima_nexus_message_type">
				<option value="general" <?php selected( $message_type, 'general' ); ?>><?php _e( 'Mensaje General', 'animaavatar' ); ?></option>
				<option value="duda" <?php selected( $message_type, 'duda' ); ?>><?php _e( 'Duda/Pregunta', 'animaavatar' ); ?></option>
				<option value="idea" <?php selected( $message_type, 'idea' ); ?>><?php _e( 'Idea/Sugerencia', 'animaavatar' ); ?></option>
				<option value="evento" <?php selected( $message_type, 'evento' ); ?>><?php _e( 'Evento/Streaming', 'animaavatar' ); ?></option>
			</select>
		</p>
		...
		<div id="nexus-event-details" style="display: <?php echo ( $message_type === 'evento' ) ? 'block' : 'none'; ?>;">
			...
			<p>
				...
				<input type="datetime-local" id="anima_nexus_event_date" name="anima_nexus_event_date" value="<?php echo esc_attr( $event_date ); ?>">
			</p>
			<p>
				<label for="anima_nexus_event_location"><?php _e( 'Ubicación/Plataforma:', 'animaavatar' ); ?></label>
				<input type="text" id="anima_nexus_event_location" name="anima_nexus_event_location" value="<?php echo esc_attr( $event_location ); ?>">
			</p>
			<p>
				<label for="anima_nexus_event_link"><?php _e( 'Enlace para Unirse:', 'animaavatar' ); ?></label>
				<input type="url" id="anima_nexus_event_link" name="anima_nexus_event_link" value="<?php echo esc_url( $event_link ); ?>">
			</p>
		</div>
...
function anima_restricted_menu_interceptor() {
	...
	<div id="anima-restricted-modal-overlay" style="display: none;">
		<div class="anima-restricted-modal-content">
			...
			<p class="sub-message">El Nexus y la Comunidad son áreas exclusivas para agentes registrados. Crea tu identidad digital para acceder.</p>
...
if ( ! function_exists('anima_get_user_courses') ) {
    function anima_get_user_courses( $user_id ) {
        ...
        if ( ! function_exists( 'wc_get_orders' ) || ! $user_id ) return [];
        ...
        foreach ( $orders as $order ) {
            foreach ( $order->get_items() as $item ) {
                ...
                if ( in_array( $main_id, $seen_products ) ) continue;
