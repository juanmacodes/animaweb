        ...
        if (!is_user_logged_in()): ?>
            ...
            <div class="nexus-access-denied-wrapper">
                <div class="cyberpunk-box access-box">
                    <span class="dashicons dashicons-lock"
                        style="font-size: 4em; color: var(--nexus-neon-pink, #f0f); margin-bottom: 20px;"></span>
                    <h1 class="entry-title glitch-text" data-text="ACCESO RESTRINGIDO">ACCESO RESTRINGIDO</h1>
                    <p class="cyber-subtitle" style="margin-bottom: 30px;">Esta zona del Nexus requiere identificación
                        neuronal.</p>

                    <h2
                        style="color: var(--nexus-neon-green, #39ff14); text-transform: uppercase; letter-spacing: 2px; margin: 40px 0;">
                        UNETE AL METAVERSO
                    </h2>
            ...
            if (function_exists('anima_nexus_get_incoming_requests_ids')) {
                ...
                if (!empty($incoming_requests_ids)): ?>
                    <section class="nexus-section incoming-transmissions">
                        <h2 class="section-title"><span class="dashicons dashicons-wifi"></span> Transmisiones Entrantes
                            (<?php echo count($incoming_requests_ids); ?>)</h2>
                        <div class="nexus-requests-list">
                            <?php foreach ($incoming_requests_ids as $requester_id):
                                $requester_data = get_userdata($requester_id);
                                if (!$requester_data)
                                    continue; ?>
                                <div class="nexus-request-card cyberpunk-box"
                                    data-requester-id="<?php echo esc_attr($requester_id); ?>">
                                    <div class="request-user-info">
                                        <?php echo get_avatar($requester_id, 50); ?>
                                        <span class="agent-name">Avatar:
                                            <strong><?php echo esc_html($requester_data->display_name); ?></strong></span>
                                        <span class="request-label">Solicita enlace neuronal...</span>
                                    </div>
                                    <div class="nexus-request-actions">
                                        <button class="anima-nexus-btn nexus-accept-btn"><span class="dashicons dashicons-yes"></span>
                                            Autorizar</button>
                                        <button class="anima-nexus-btn nexus-block-btn"><span class="dashicons dashicons-no"></span>
                                            Bloquear Señal</button>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
            ...
            <section class="nexus-section all-agents-directory">
                ...
                <div class="nexus-filter-bar cyberpunk-box">
                    <form method="get" action="<?php echo esc_url(get_permalink()); ?>" class="nexus-filter-form"
                        id="nexus-filter-form">
                        <div class="filter-group status-filter">
                            <label for="nexus_filter" class="filter-label"><span class="dashicons dashicons-filter"></span>
                                Modo de Visualización:</label>
                            <select name="nexus_filter" id="nexus_filter" class="cyber-input">
                                <option value="all" <?php selected($filter_status, 'all'); ?>>🌐 Toda la Red Global
                                </option>
                                <option value="friends" <?php selected($filter_status, 'friends'); ?>>🧠 Mis Enlaces
                                    Neuronales (Amigos)</option>
                            </select>
                        </div>
                        <div class="filter-group rank-filter">
                            <label for="nexus_rank" class="filter-label"><span class="dashicons dashicons-awards"></span>
                                Filtrar por Rango:</label>
                            <select name="nexus_rank" id="nexus_rank" class="cyber-input">
                                <option value="all">Todos los Rangos</option>
                                <option value="Novato">Novato</option>
                                <option value="Iniciado">Iniciado</option>
                                <option value="Hacker">Hacker</option>
                                <option value="Cyber-Agent">Cyber-Agent</option>
                                <option value="Netrunner">Netrunner</option>
                                <option value="Neon Samurai">Neon Samurai</option>
                                <option value="Cyberlord">Cyberlord</option>
                            </select>
                        </div>
                        <div class="filter-group search-filter">
                            <label for="nexus_search" class="filter-label"><span class="dashicons dashicons-search"></span>
                                Buscar Avatar:</label>
                            <div class="search-input-wrapper">
                                <input type="text" name="nexus_search" id="nexus_search" class="cyber-input"
                                    placeholder="Nombre del avatar..." value="<?php echo esc_attr($search_term); ?>">
                ...
                <div id="nexus-results-container">
                    <div class="agents-grid nexus-grid-layout">
                        ...
                        if (!empty($avatars)) {
                                        ...
                                        <?php switch ($status):
                                            case 'none': ?>
                                                <button class="anima-nexus-btn anima-nexus-connect-btn full-width"
                                                    data-recipient-id="<?php echo esc_attr($agent_id); ?>"><span
                                                        class="dashicons dashicons-networking"></span> Establecer Enlace</button>
                                                <?php break;
                                            case 'pending_sent': ?>
                                            ...
                                            case 'pending_received': ?>
                                                <span class="nexus-status-label pending-label"><span
                                                        class="dashicons dashicons-arrow-down-alt"></span> Solicitud recibida</span>
                                                <?php break;
                                            case 'accepted': ?>
                                                <span class="nexus-status-label connected-label"><span
                                                        class="dashicons dashicons-yes-alt"></span> Enlace Neuronal Activo</span>
                                                <?php break;
                                        endswitch; ?>
                                    </div>
                                </div>
                            ...
                            if ($filtering_by_friends && empty($friend_ids)) {
                                echo '<p>Aún no tienes Enlaces Neuronales activos. ¡Explora la Red Global para conectar!</p>';
                            } elseif (!empty($search_term)) {
                                echo '<p>No se encontraron avatares que coincidan con la señal de búsqueda: "' . esc_html($search_term) . '".</p>';
                            } else {
                                echo '<p>No se detectan datos en este sector de la red.</p>';
                            }
                            echo '</div>';
                        }
                        ?>
            ...
            <script>
                jQuery(document).ready(function ($) {
                    function loadNexusResults(paged = 1) {
                        var filter = $('#nexus_filter').val();
                        var rank = $('#nexus_rank').val();
                        var search = $('#nexus_search').val();

                        $('#nexus-results-container').css('opacity', '0.5');

                        $.ajax({
                            url: anima_nexus_vars.ajax_url,
                            type: 'POST',
                            data: {
                                action: 'anima_nexus_filter_users',
                                filter: filter,
                                rank: rank,
                                search: search,
                                paged: paged
                            ...
                            success: function (response) {
                                if (response.success) {
                                    var html = '<div class="agents-grid nexus-grid-layout">' + response.data.html + '</div>';
                                    if (response.data.pagination) {
                                        html += '<div class="nexus-pagination-container">' + response.data.pagination + '</div>';
                                    }
                                    $('#nexus-results-container').html(html).css('opacity', '1');
                                }
                            }
                        });
                    }

                    $('#nexus-filter-form').on('submit', function (e) {
                        e.preventDefault();
                        loadNexusResults(1);
                    });

                    $('#nexus_filter, #nexus_rank').on('change', function () {
                        loadNexusResults(1);
                    });

                    $(document).on('click', '.nexus-pagination-container a', function (e) {
                        e.preventDefault();
                        ...
                        if (match) paged = match[1];
                            ...
                            if (urlParams.has('paged')) paged = urlParams.get('paged');
                        }
                        loadNexusResults(paged);
                    });

                    // --- MANEJO DE BOTONES DE CONEXIÓN (Delegación para AJAX) ---
                    $(document).on('click', '.anima-nexus-connect-btn', function (e) {
                        e.preventDefault();
                        var $button = $(this);
                        ...
                        $button.addClass('processing').prop('disabled', true).html('<span class="glitch">Transmitiendo...</span>');

                        $.ajax({
                            url: anima_nexus_vars.ajax_url,
                            type: 'POST',
                            data: {
                                action: 'anima_nexus_send_request',
                            ...
                            success: function (response) {
                                if (response.success) {
                                    $button.removeClass('processing')
                                        .addClass('anima-nexus-pending-btn disabled')
                                        .html('<span class="dashicons dashicons-hourglass"></span> Señal en Espera');
                                } else {
                                    $button.removeClass('processing').prop('disabled', false).html(originalText);
                    ...
                    // --- MANEJO DE RESPUESTAS (Aceptar / Bloquear) ---
                    $(document).on('click', '.nexus-accept-btn, .nexus-block-btn', function (e) {
                        e.preventDefault();
                        var $button = $(this);
                        var $card = $button.closest('.nexus-request-card');
                        ...
                        $card.addClass('processing').css('opacity', '0.5');
                            ...
                            success: function (response) {
                                if (response.success) {
                                    $card.slideUp(300, function () {
                                        $(this).remove();
                                        if ($('.nexus-request-card').length === 0) {
                                            $('.incoming-transmissions').fadeOut();
                                        }
                                    });
                                } else {
...
<style>
    ...
    .section-title {
        border-bottom: 2px solid var(--nexus-neon-blue, #0ff);
        padding-bottom: 10px;
        margin-bottom: 30px;
        display: flex;
        align-items: center;
    ...
    .cyberpunk-box:hover {
        border-color: var(--nexus-neon-blue, #0ff);
    ...
    .agent-card .avatar {
        border-radius: 50%;
        border: 3px solid var(--nexus-neon-pink, #f0f);
        margin-bottom: 15px;
    }
    ...
    .connected-label {
        ...
        border: 1px solid var(--nexus-neon-green, #39ff14);
    ...
    .cyber-input {
        ...
        border: 1px solid var(--nexus-neon-blue, #0ff);
        color: #fff;
        padding: 10px 15px;
        font-family: 'Rajdhani', sans-serif;
        outline: none;
    ...
    .cyber-input:focus {
        box-shadow: 0 0 10px var(--nexus-neon-blue, #0ff);
    ...
    /* Paginación */
    .nexus-pagination-container {
        display: flex;
        justify-content: center;
        margin-top: 40px;
    }

    .nexus-pagination-container ul.page-numbers {
        display: flex;
        list-style: none;
        padding: 0;
        margin: 0;
    ...
    .nexus-pagination-container .page-numbers a,
    .nexus-pagination-container .page-numbers span.dots {
        display: flex;
        align-items: center;
        padding: 10px 15px;
        background: rgba(10, 10, 10, 0.8);
    ...
    .nexus-pagination-container .page-numbers a:hover {
        background: rgba(0, 255, 255, 0.2);
        box-shadow: 0 0 10px var(--nexus-neon-blue, #0ff);
    }

    .nexus-pagination-container .page-numbers span.current {
        display: flex;
        align-items: center;
        padding: 10px 15px;
        background: var(--nexus-neon-blue, #0ff);
        color: #000;
        border: 1px solid var(--nexus-neon-blue, #0ff);
        font-weight: bold;
        box-shadow: 0 0 15px var(--nexus-neon-blue, #0ff);
    }
