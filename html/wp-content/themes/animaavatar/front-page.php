...
?>
<section class="hero dark-bg relative">
    <div id="hero-canvas-container" class="hero-canvas"></div>
    <div class="container relative z-10">
        <span
            class="badge pill-gradient animate-on-scroll"><?php esc_html_e('Laboratorio inmersivo', 'animaavatar'); ?></span>
        <h1 class="hero__title animate-on-scroll">
            <?php esc_html_e('Diseñamos avatares que brillan en cualquier realidad', 'animaavatar'); ?></h1>
        <p class="hero__subtitle animate-on-scroll">
            <?php esc_html_e('Tecnología XR, pipelines realtime y storytelling volumétrico para marcas que buscan impacto en streaming, VR y metaverso.', 'animaavatar'); ?>
        </p>
        <div class="flex flex-center animate-on-scroll">
            <a class="button"
                href="<?php echo esc_url(home_url('/contacto')); ?>"><?php esc_html_e('Agenda una demo', 'animaavatar'); ?></a>
            <a class="button"
                href="<?php echo esc_url(home_url('/experiencia-inmersiva')); ?>"><?php esc_html_e('Explora la experiencia 3D', 'animaavatar'); ?></a>
        </div>
    </div>
</section>

<section class="anima-live-teaser">
    <div class="container relative z-10">
        <span class="anima-live-badge animate-on-scroll"><?php esc_html_e('Coming Soon', 'animaavatar'); ?></span>
        <h2 class="animate-on-scroll"><?php esc_html_e('Anima Live: Tu Identidad Digital', 'animaavatar'); ?></h2>
        <p class="animate-on-scroll">
            <?php esc_html_e('La primera plataforma descentralizada para gestionar, personalizar y monetizar tus avatares en el metaverso. Conecta con Unreal Engine en tiempo real.', 'animaavatar'); ?>
        </p>
        <a class="button animate-on-scroll" href="#"><?php esc_html_e('Unirse a la Waitlist', 'animaavatar'); ?></a>
    </div>
</section>

<section class="section container" aria-labelledby="slider-title">
    <div class="flex flex-between animate-on-scroll">
        <h2 id="slider-title" class="section__title"><?php esc_html_e('Experiencias recientes', 'animaavatar'); ?>
        </h2>
    ...
    <div class="swiper-container overflow-hidden animate-on-scroll">
        <div class="swiper" id="home-slider" aria-live="polite"
            aria-label="<?php esc_attr_e('Slider de experiencias recientes', 'animaavatar'); ?>">
            <div class="swiper-wrapper">
                ...
                if ($slides->have_posts()):
                    ...
                    ?>
                    <p><?php esc_html_e('Añade diapositivas desde el panel de administración para alimentar este slider.', 'animaavatar'); ?>
                    </p>
                    <?php
                endif;
                ?>
            ...
            <div class="swiper-navigation" aria-controls="home-slider">
                <button type="button" class="swiper-button-prev"
                    aria-label="<?php esc_attr_e('Ver diapositiva anterior', 'animaavatar'); ?>"
                    aria-controls="home-slider">
                    <span class="screen-reader-text"><?php esc_html_e('Anterior', 'animaavatar'); ?></span>
                </button>
                <button type="button" class="swiper-button-next"
                    aria-label="<?php esc_attr_e('Ver diapositiva siguiente', 'animaavatar'); ?>"
                    aria-controls="home-slider">
        ...
        if (!empty($curso_ids)) {
            foreach ($curso_ids as $curso_id) {
                ...
                <article class="card animate-on-scroll">
                    <header>
                        <h3><a
                                href="<?php echo esc_url(get_permalink($curso)); ?>"><?php echo esc_html(get_the_title($curso)); ?></a>
                        </h3>
                    </header>
                    <p><?php echo esc_html(wp_trim_words(get_the_excerpt($curso), 20)); ?></p>
                    <a class="button"
                        href="<?php echo esc_url(get_permalink($curso)); ?>"><?php esc_html_e('Ver curso', 'animaavatar'); ?></a>
                </article>
                <?php
            }
        } else {
        ...
        if (!empty($avatar_ids)) {
            foreach ($avatar_ids as $avatar_id) {
                ...
                <article class="card animate-on-scroll">
                    ...
                    <header>
                        <h3><a
                                href="<?php echo esc_url(get_permalink($avatar)); ?>"><?php echo esc_html(get_the_title($avatar)); ?></a>
                        </h3>
                    </header>
                    <p><?php echo esc_html(wp_trim_words(get_the_excerpt($avatar), 18)); ?></p>
                    <a class="button"
                        href="<?php echo esc_url(get_permalink($avatar)); ?>"><?php esc_html_e('Ver avatar', 'animaavatar'); ?></a>
                </article>
                <?php
            }
        } else {
...
<section class="section dark-bg">
    <div class="container text-center animate-on-scroll">
        ...
        <p><?php esc_html_e('Agenda una consultoría express o sumérgete en nuestra experiencia preparada con Unreal Engine.', 'animaavatar'); ?>
        </p>
        <a class="button"
            href="<?php echo esc_url(home_url('/experiencia-inmersiva')); ?>"><?php esc_html_e('Ir a Experiencia Inmersiva', 'animaavatar'); ?></a>
    </div>
</section>
<?php
get_footer();
